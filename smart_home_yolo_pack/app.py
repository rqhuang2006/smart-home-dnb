...
# Flask API / GUI
# =============================
def build_v1_detection_payload(data: Dict[str, Any], base_url: str) -> Dict[str, Any]:
    """Convert the YOLO module result to the shared C-backend response shape."""
    objects = []
    ...
    light_bulb_detected = any(
        str(item.get("label", "")).strip().lower().replace("-", "_").replace(" ", "_")
        in {"light_bulb", "bulb", "灯泡"}
        for item in objects
    )
    ...
    return {
        ...
        "image_path": data.get("raw_image"),
        "result_image_path": data.get("result_image"),
        "image": {
        ...
        "trigger_action": {
            "type": "light_on",
            "executed": light_bulb_detected,
            "reason": "detected light_bulb" if light_bulb_detected else "no light_bulb detected",
        },
...
def api_v1_vision_detect():
    ...
    try:
        ...
        return jsonify({
            "code": 0,
            "message": "ok",
            "data": build_v1_detection_payload(data, base_url),
        })
...
@app.route("/api/v1/vision/records", methods=["GET"])
def api_v1_vision_records():
    """Shared interface for GUI/C-backend polling YOLO detection history."""
    limit = int(request.args.get("limit", 20))
    base_url = request.host_url.rstrip("/")
    records = [build_v1_detection_payload(item, base_url) for item in get_history(limit=limit)]
    return jsonify({"code": 0, "message": "ok", "data": records})
